...
class LocationSensor:
    def __init__(self, zone=52):
        ### 심 버전 / real world 버전 스위칭
        self.gps_sub = rospy.Subscriber("/gps", GPSMessage, self.gps_callback)
        # self.gps_sub = rospy.Subscriber("/gps_data/fix", NavSatFix, self.gps_callback)

        self.odom_pub = rospy.Publisher("/gps_utm_odom", Odometry, queue_size=10)
        ...
        self.prev_y=0
        self.prev_heading=0
        self.east_offset = 302595.0
        self.north_offset = 4124145.0
    ...
    def gps_callback(self, gps_msg):
        ...
        e_o = self.east_offset   
        n_o = self.north_offset
