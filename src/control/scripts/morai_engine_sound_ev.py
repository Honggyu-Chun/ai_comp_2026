...
class SampleEVEngine:
    """Replay a 0-to-max-speed EV acceleration sample by vehicle speed."""
    ...
    def __init__(
        ...
        sample_end_time_s=None,
        sample_speed_min_kph=0.0,
        sample_speed_max_kph=None,
        position_tau_s=0.10,
        gain=1.0,
        playback_rate=1.0,
    ):
        ...
        self.max_speed_kph = max(0.1, float(max_speed_kph))
        self.sample_speed_min_kph = float(sample_speed_min_kph)
        self.sample_speed_max_kph = (
            self.max_speed_kph if sample_speed_max_kph is None else float(sample_speed_max_kph)
        )
        self.position_tau_s = max(1e-3, float(position_tau_s))
        ...
        with wave.open(sample_path, "rb") as wav_file:
            ...
            self.channels = int(wav_file.getnchannels())
            self.sample_width = int(wav_file.getsampwidth())
            total_frames = int(wav_file.getnframes())
            raw = wav_file.readframes(total_frames)

        if self.sample_width != 2:
            raise RuntimeError("sample_ev only supports 16-bit PCM WAV: {}".format(sample_path))
        if self.channels not in (1, 2):
            raise RuntimeError("sample_ev only supports mono/stereo WAV: {}".format(sample_path))
        ...
        self.audio = audio
        self.total_frames = int(audio.shape[0])
        self.duration_s = self.total_frames / float(self.sample_rate)

        self.start_frame = int(max(0.0, float(sample_start_time_s)) * self.sample_rate)
        if sample_end_time_s is None or float(sample_end_time_s) <= 0.0:
            self.end_frame = self.total_frames - 2
        else:
            self.end_frame = int(float(sample_end_time_s) * self.sample_rate)

        self.start_frame = int(clamp(self.start_frame, 0, max(0, self.total_frames - 2)))
        self.end_frame = int(clamp(self.end_frame, self.start_frame + 1, self.total_frames - 2))
        self.grain_count = int(clamp(
            int(rospy.get_param("~sample_grain_count", 4)), 2, 8
        ))
        grain_ms = max(80.0, float(rospy.get_param("~sample_grain_ms", 320.0)))
        self.grain_frames = max(256, int(grain_ms * self.output_sample_rate / 1000.0))
        self.grain_hop = max(64, self.grain_frames // self.grain_count)
        jitter_ms = max(0.0, float(rospy.get_param("~sample_grain_jitter_ms", 28.0)))
        self.jitter_frames = jitter_ms * self.sample_rate / 1000.0
        self.jitter_tau_s = max(
            0.05, float(rospy.get_param("~sample_jitter_tau_s", 0.35))
        )
        self.rate_jitter = clamp(
            float(rospy.get_param("~sample_grain_rate_jitter", 0.008)), 0.0, 0.05
        )
        self.throttle_hf_gain = max(
            0.0, float(rospy.get_param("~sample_throttle_hf_gain", 0.45))
        )
        self.regen_gain = max(
            0.0, float(rospy.get_param("~sample_regen_gain", 0.30))
        )

        self.rng = np.random.RandomState(2026)
        self.grains = []
        self.next_grain_in = self.grain_hop
        self.smoothed_target_frame = float(self.start_frame)
        self.jitter_value = 0.0
        self.jitter_target = 0.0
        self.jitter_update_hops = max(
            1, int(0.25 * self.output_sample_rate / self.grain_hop)
        )
        self.jitter_hops_left = 0
        self.smoothed_throttle = 0.0
        self.smoothed_regen = 0.0
        self.hf_lowpass_state = np.zeros(2, dtype=np.float64)

        # Seed four staggered grains so the first callback starts with a full
        # overlap instead of fading in one grain at a time.
        for index in range(self.grain_count):
            self._spawn_grain(index * self.grain_hop, 1.0)

    def target_frame_for_speed(self, speed_kph):
        denom = max(0.1, self.sample_speed_max_kph - self.sample_speed_min_kph)
        speed_ratio = clamp((float(speed_kph) - self.sample_speed_min_kph) / denom, 0.0, 1.0)
        return self.start_frame + speed_ratio * (self.end_frame - self.start_frame)

    def _spawn_grain(self, age, steady_amount):
        if self.jitter_hops_left <= 0:
            self.jitter_target = self.rng.uniform(-1.0, 1.0) * self.jitter_frames
            self.jitter_hops_left = self.jitter_update_hops
        self.jitter_hops_left -= 1

        hop_dt = self.grain_hop / float(self.output_sample_rate)
        jitter_alpha = 1.0 - math.exp(-hop_dt / self.jitter_tau_s)
        self.jitter_value += jitter_alpha * (
            self.jitter_target - self.jitter_value
        )
        jitter = self.jitter_value * clamp(steady_amount, 0.0, 1.0)
        rate = self.playback_rate * (
            1.0 + self.rng.uniform(-self.rate_jitter, self.rate_jitter)
        )
        source_span = self.grain_frames * self.sample_rate / float(
            self.output_sample_rate
        ) * rate
        source_start = self.smoothed_target_frame + jitter - 0.5 * source_span
        source_start = clamp(
            source_start,
            self.start_frame,
            max(self.start_frame, self.end_frame - source_span - 2.0),
        )
        self.grains.append({
            "age": int(age),
    ...
    def _read_linear(self, positions):
        positions = np.clip(positions, self.start_frame, self.end_frame - 2)
        i0 = np.floor(positions).astype(np.int32)
        frac = (positions - i0).reshape(-1, 1)
        return self.audio[i0] * (1.0 - frac) + self.audio[i0 + 1] * frac

    def _render_grains(self, frames, steady_amount):
        # Schedule new grains at exact sample offsets even when a grain boundary
        # falls in the middle of an audio callback.
        while self.next_grain_in < frames:
            self._spawn_grain(-self.next_grain_in, steady_amount)
            self.next_grain_in += self.grain_hop
        self.next_grain_in -= frames

        output = np.zeros((frames, 2), dtype=np.float64)
        envelope_sum = np.zeros(frames, dtype=np.float64)
        sample_offsets = np.arange(frames, dtype=np.int32)
        alive = []

        for grain in self.grains:
            ages = grain["age"] + sample_offsets
            active = (ages >= 0) & (ages < self.grain_frames)
            if np.any(active):
                active_ages = ages[active].astype(np.float64)
                phase = active_ages / float(self.grain_frames - 1)
                envelope = 0.5 - 0.5 * np.cos(2.0 * math.pi * phase)
                source_positions = grain["source_start"] + (
                    active_ages * self.sample_rate / float(self.output_sample_rate)
                    * grain["rate"]
                )
                output[active] += self._read_linear(source_positions) * envelope.reshape(-1, 1)
                envelope_sum[active] += envelope
        ...
        self.grains = alive
        output /= np.maximum(envelope_sum, 1e-5).reshape(-1, 1)
        return output

    def _split_high_frequency(self, samples):
        cutoff_hz = 1100.0
        alpha = 1.0 - math.exp(
            -2.0 * math.pi * cutoff_hz / self.output_sample_rate
        )
        low = np.empty_like(samples)
        state = self.hf_lowpass_state.copy()
        for index in range(len(samples)):
            state += alpha * (samples[index] - state)
            low[index] = state
        self.hf_lowpass_state = state
        return low, samples - low

    def render(self, frames, speed_kph, throttle, brake, signal_gain):
        frames = int(frames)
        throttle = clamp(float(throttle), 0.0, 1.0)
        brake = clamp(float(brake), 0.0, 1.0)

        chunk_dt = frames / float(self.output_sample_rate)
        target_frame = self.target_frame_for_speed(speed_kph)
        position_alpha = 1.0 - math.exp(-chunk_dt / self.position_tau_s)
        position_error = abs(target_frame - self.smoothed_target_frame)
        self.smoothed_target_frame += position_alpha * (
            target_frame - self.smoothed_target_frame
        )
        steady_amount = 1.0 - clamp(
            position_error / max(1.0, 0.40 * self.sample_rate), 0.0, 1.0
        )
        samples = self._render_grains(frames, steady_amount)

        control_alpha = 1.0 - math.exp(-chunk_dt / 0.10)
        self.smoothed_throttle += control_alpha * (
            throttle - self.smoothed_throttle
        )
        speed_presence = clamp(float(speed_kph) / 5.0, 0.0, 1.0)
        coast_regen = (
            clamp((0.12 - throttle) / 0.12, 0.0, 1.0)
            * speed_presence * 0.65
        )
        target_regen = max(brake, coast_regen)
        self.smoothed_regen += control_alpha * (
            target_regen - self.smoothed_regen
        )

        low, high = self._split_high_frequency(samples)
        hf_emphasis = self.throttle_hf_gain * self.smoothed_throttle
        regen_layer = high * self.regen_gain * self.smoothed_regen
        samples = low + high * (1.0 + hf_emphasis) + regen_layer

        drive_gain = (
            0.28 + 0.52 * self.smoothed_throttle + 0.20 * speed_presence
        )

        output_gain = self.volume * self.gain * drive_gain * clamp(signal_gain, 0.0, 1.0)
        samples = np.tanh(samples * output_gain * 1.05) * 0.82

        stereo = np.empty(frames * 2, dtype="<i2")
        samples = np.clip(samples, -1.0, 1.0)
        stereo[0::2] = np.asarray(samples[:, 0] * 32767.0, dtype="<i2")
        stereo[1::2] = np.asarray(samples[:, 1] * 32767.0, dtype="<i2")
...
class AudioSink:
    def __init__(self, backend, sample_rate, latency_ms, process_time_ms):
        ...
        # paplay normally provides pacing by blocking this pipe. That also makes
        # Ctrl+C wait behind a blocked write, so pace in the ROS loop instead.
    ...
    def write(self, pcm):
        with self.io_lock:
            ...
            if self.process.poll() is not None:
                raise RuntimeError(
                    "audio backend terminated with code {}".format(
            ...
            try:
                ...
                # The backend already has enough buffered audio. Dropping one
                # chunk is preferable to blocking ROS shutdown indefinitely.
                return
...
class MoraiEngineSoundNode:
    def __init__(self):
        ...
        if self.engine_style == "sample_ev":
            self.sample_audio_path = rospy.get_param("~sample_audio_path")
            self.sample_start_time_s = float(rospy.get_param("~sample_start_time_s", 0.0))
            self.sample_end_time_s = float(rospy.get_param("~sample_end_time_s", -1.0))
            self.sample_speed_min_kph = float(rospy.get_param("~sample_speed_min_kph", 0.0))
            self.sample_speed_max_kph = float(
                rospy.get_param("~sample_speed_max_kph", self.powertrain.max_speed_kph)
            )
            self.sample_position_tau_s = float(rospy.get_param("~sample_position_tau_s", 0.10))
            self.sample_gain = float(rospy.get_param("~sample_gain", 1.0))
            self.sample_playback_rate = float(rospy.get_param("~sample_playback_rate", 1.0))
            sample_end = None if self.sample_end_time_s <= 0.0 else self.sample_end_time_s
            self.synth = SampleEVEngine(
                self.sample_audio_path,
                self.sample_rate,
                self.volume,
                self.powertrain.max_speed_kph,
                self.sample_start_time_s,
                sample_end,
                self.sample_speed_min_kph,
                self.sample_speed_max_kph,
                self.sample_position_tau_s,
                self.sample_gain,
                self.sample_playback_rate,
            )
            rospy.loginfo(
                "[engine_sound] sample_ev loaded=%s duration=%.3fs range=%.1f~%.1f kph",
                self.sample_audio_path, self.synth.duration_s,
                self.sample_speed_min_kph, self.sample_speed_max_kph,
            )
        else:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.engine_style == "sample_ev":
                # EV sample mode has no virtual transmission or RPM-matching
                # state. These values are only kept for the existing status topics.
                rpm, gear, load, shift_kick = 0.0, 1, throttle, 0.0
                pcm = self.synth.render(
                    self.frames_per_chunk, speed_kph, throttle, brake, self.signal_gain
                )
            else:
            ...
            if now >= next_status_time:
                ...
                if self.engine_style == "sample_ev":
                    shift_label = "EV_MODE"
                elif self.engine_style == "ev_spaceship":
                    shift_label = "EV_DIRECT"
                else:
            ...
            # Explicit real-time pacing is required because the audio pipe is
            # deliberately non-blocking for immediate Ctrl+C shutdown.
